...
$(document).ready(function () {
  ...
  function fetchReviews(placeId) {
    $.getJSON(`http://localhost:5001/api/v1/places/${placeId}/reviews`, (data) => {
      const reviewsContainer = $(`.reviews[data-place="${placeId}"] ul`);
      const toggleReview = $(`.reviews[data-place="${placeId}"] h2 #toggle_review`);

      if (toggleReview.text() === "show") {
        toggleReview.text("hide");

        data.forEach((review) => {
          $.getJSON(`http://localhost:5001/api/v1/users/${review.user_id}`, (user) => {
      ...
      } else if (toggleReview.text() === "hide") {
